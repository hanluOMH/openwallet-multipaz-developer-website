...
WebVerifierRequestCredentialsAndroidSystemRouteUserAuthorizeWalletPresentVerifierValidate
...
This method configures all the necessary components for W3C DC API operation.
...
The `privilegedUserAgents.json` file contains a list of trusted applications that can request credentials:
...
#### **DigitalCredentials.Default Integration**
...
1. **Availability Check**: Verifies that the Android Digital Credentials API is available on the device
2. **Credential Export**: Makes stored credentials available to the W3C DC API system
3. **Document Store Integration**: Connects your wallet's credential storage to the Android credential infrastructure
4. **Multi-Presentation Support**: Enables credentials to be used for both W3C DC API and other presentation methods (QR, NFC, BLE)

### **Step 5: Testing the Implementation**

####Testing with Web Verifiers**

To test your W3C DC API implementation:

1. **Deploy Your App**: Install the wallet app on an Android device
2. **Open Web Verifier**: Navigate to a W3C DC API compatible verifier (like https://verifier.multipaz.org/)
